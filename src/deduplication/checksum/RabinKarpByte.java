...
public class RabinKarpByte {
    private byte[] pat;      // the pattern  // needed only for Las Vegas
    ...
    private long hash(byte[] key, int M) { 
        long h = 0; 
        for (int j = 0; j < M; j++) 
            h = (R * h + key[j]) % Q; 
        return h; 
    ...
    private boolean check(byte[] txt, int i) {
        for (int j = 0; j < M; j++) 
            if (txt[j] != txt[i + j]) 
                return false; 
        return true;
